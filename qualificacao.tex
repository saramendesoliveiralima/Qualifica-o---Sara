\documentclass[pt, qual, classic, oneside, a4paper, scr]{ufba/ufbathesis}

\usepackage[utf8]{inputenc}
%%\usepackage[ansinew]{inputenc}
\usepackage{graphicx}
\usepackage{lipsum}
\usepackage{hyphenat}
\usepackage[usenames, dvipsnames, table]{xcolor}
\usepackage{booktabs}
\usepackage{pifont}
\usepackage{multicol}
\usepackage{multirow}
\usepackage{listings} 
\usepackage{colortbl}
\usepackage{xfrac}
\usepackage[FIGTOPCAP]{subfigure}
\usepackage[printonlyused, withpage]{acronym}
\usepackage{enumitem}
\usepackage{longtable}
\usepackage{lscape}
\usepackage{todonotes}
\usepackage{mathtools}
\usepackage{verbatim}



\university{Universidade Federal da Bahia}
\address{Salvador}
\institute{Instituto de Matem\'{a}tica e Estatística}
\library{Biblioteca Reitor Macedo Costa}
\program{Programa de Pós-Graduação em Ciência da Computação}
\majorfield{Ciência da Computação}
\title{APLICAÇÃO DAS TÉCNICAS DE SELEÇÃO DE TESTE DE REGRESSÃO NO DESENVOLVIMENTO DE APPS ANDROID}
\date{Dezembro/2020}
\defenseyear{2020}
\author{Sara Mendes Oliveira Lima}
\adviser{Prof. Dr. Ivan do Carmo Machado}
\coadviser{Profª. Dra. Larissa Rocha Soares}

\begin{document}
\renewcommand{\bibname}{Referências}
\pgcompfrontpage

% Parte pre-textual
\frontmatter
\pgcomppresentationpage
\include{preambulo/abstract}

\tableofcontents

\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{\listfigurename}
\listoffigures

\cleardoublepage
\phantomsection
\listoftables
\addcontentsline{toc}{chapter}{Lista de Tabelas}

\input{preambulo/siglas.tex}


\mainmatter

\include{chapters/introduction}
\include{chapters/background}
\include{chapters/survey}
\include{chapters/considerations}

\backmatter

\bibliographystyle{ufba/abntex2-alf}
\bibliography{biblio}

\input{apendices/apendice}

\end{document}





...
Esta questão foi refinada em duas questões específicas:
\vspace{.5em}

\begin{enumerate}[label=\bf QP\arabic*,leftmargin=1.8cm]
    
    \item Quais são as técnicas de seleção de teste de regressão apresentadas na literatura?
    
    \item \textcolor{blue}{Como a indústria brasileira de testes de \ac{APPS} Android realiza teste de regressão?}
...
Compreendendo que o estudo das técnicas de teste de regressão para Android é um campo em expansão, o presente trabalho tem como objetivo \textbf{realizar uma avaliação empírica das técnicas de teste de regressão \textcolor{blue}{implementadas por ferramentas} aplicadas a \ac{APPS} Android.}
...
\begin{enumerate}[label=\bf O\arabic*,leftmargin=1.5cm]
    ...
    \item \textcolor{blue}{Realizar um estudo das ferramentas de teste de regressão para Android. Para alcançar esse objetivo, será realizado uma revisão estruturada da literatura sobre ferramentas de teste de software automatizado para Android, e, sobre ferramentas de teste de regressão, em busca de alguma ferramenta que faça teste de regressão para Android ou que possa ser adaptada para esse propósito.}

   \item \textcolor{blue}{Levantar como a indústria de \ac{APPS} Android realiza teste de regressão. Para alcançar esse objetivo, será realizado um Survey.}
    
   \item \textcolor{blue}{Realizar uma análise comparativa das técnicas de seleção de testes de regressão implementadas por ferramentas aplicadas a \ac{APPS} Android. Para alcançar esse objetivo, será realizado um estudo experimental.}
...
\begin{itemize}
  ...
  \item \textbf{Fase 2}: Refere-se ao Estudo Experimental.
  
  Esta etapa contempla a execução de um experimento cujo objetivo principal é avaliar empiricamente a eficiência das técnicas de teste de regressão para \ac{APPS} implementadas por ferramentas, em um conjunto de \ac{APPS} Android, vide Tabela \ref{table:apps}. O estudo experimental foi planejado com base em \cite{Wohlin:2012:ESE:2349018}.
  
  
  \item \textbf{Fase 3}: \textcolor{blue}{Refere-se ao Survey.}
  
  \textcolor{blue}{Esta etapa contempla a aplicação de um Survey em empresas de desenvolvimento/testes de \ac{APPS} Android, para compreender como a indústria brasileira de teste realiza teste de regressão. O Survey foi planejado com base em \cite{Kitchenham:2002:PSR:566493.566495}.}
...
\begin{itemize}
    ...
    \item Contribuir com o corpo de conhecimento sobre o uso de técnicas de teste de regressão para o desenvolvimento de \ac{APPS} Android, com base na síntese de evidências disponíveis na literatura.
    
    \item Coletar e analisar dados, no sentido de prover evidências empíricas sobre \textcolor{blue}{quais as técnicas de teste de regressão implementadas por ferramentas são mais adequadas para projetos de \ac{APPS} Android.}
    
    \item \textcolor{blue}{Identificar como a indústria de \ac{APPS} Android realiza teste de regressão .}
...
\item \textbf{Capítulo 3}: Estudo Experimental;
\item \textbf{Capítulo 4}: \textit {\textcolor{blue}{Survey;}}
\item \textbf{Capítulo 5}: Considerações Finais.
\end{itemize}